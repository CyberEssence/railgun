...
func (s *TwoFAService) VerifySetup(ctx context.Context, token string, userID string) (bool, error) {
	...
	if valid {
		log.Printf("2FA setup verified successfully for user %d", userID)
	}
